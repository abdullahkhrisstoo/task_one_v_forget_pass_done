...
namespace task_one_v2.App_Core.StateMangement
{
    ...
    {
        Task<UserInfo> GetCustomerById(decimal id);
        Task<UserInfo> GetChefById(decimal id);
    ...
    public class CurrentAdminManager : IAdminManager
    {
        ...
        public async Task<UserInfo> GetCustomerById(decimal id)
        {
            var userData = await GetAllUserDependsOnUserRole();
            return userData.SingleOrDefault(check => check.Userid == id)??new UserInfo();
        }

        public async Task<UserInfo> GetChefById(decimal id)
        {
            var userData = await GetAllChefDependsOnUserRole();
            return userData.SingleOrDefault(check => check.Userid == id) ?? new UserInfo();
        }
    }
}
