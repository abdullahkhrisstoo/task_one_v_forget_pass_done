...
namespace task_one_v2.Controllers
{
    public class AdminController : Controller
    {
        ...
        {
            List<Testimonial> allTestimonial =await _adminManager.getAllTestimonials();
            return View(allTestimonial);
        }

        public async Task<IActionResult> AllContactUs()
        {
        ...
        {
            var modelContext = await _adminManager.getAllOrder();
            return View(modelContext);
        }

        public async Task<IActionResult> CustomerProfile(decimal? id)
        {
            if (id==null) return NotFound();
            return View(await _adminManager.GetCustomerById(id??0));
        }

        public async Task<IActionResult> ChefProfile(decimal? id)
        {
            if (id == null) return NotFound();
            return View(await _adminManager.GetChefById(id ?? 0));
        }
    }
