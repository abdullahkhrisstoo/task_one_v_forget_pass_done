...
namespace task_one_v2.Controllers
{
    public class ChefController : Controller
    {
        ...
        private int? _GetCurrentUserId() => _currentChefManager.GetCurrentUserId();

         private UserInfo? _GetCurrentUserData()
        {
            if (_GetCurrentUserId() != null)
            {
                UserInfo user = _currentChefManager.GetCurrentUserData();
                return user;
            }
        ...
        {
            ViewBag.currentRecipe = await _currentChefManager.GetRecipeByChefId(_GetCurrentUserId()??0);
        ...
        public async Task<IActionResult> AddRecipe(Recipe recipe)
        {
            ...
            if (_GetCurrentUserId().HasValue)
            {
                recipe.ChefId = (decimal)_GetCurrentUserId().Value;
                recipe.ApprovalStatusId = 1;
            }
            else {
